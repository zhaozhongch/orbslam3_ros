...
{
public:
    ImageGrabber(ORB_SLAM3::System* pSLAM, ImuGrabber *pImuGb, const bool bClahe,ros::NodeHandle& nh): mpSLAM(pSLAM), mpImuGb(pImuGb), mbClahe(bClahe){pub_pose_current_frame_ = nh.advertise<nav_msgs::Odometry>("/tesse/odom",10);}
...
int main(int argc, char **argv)
{
  ...
  ImuGrabber imugb;
  ImageGrabber igb(&SLAM,&imugb,bEqual, n); // TODO
...
void ImageGrabber::SyncWithImu()
{
  while(1)
  {
    ...
    if (!img0Buf.empty()&&!mpImuGb->imuBuf.empty())
    {
      ...
      ros::Time tIm_ros = img0Buf.front()->header.stamp;
